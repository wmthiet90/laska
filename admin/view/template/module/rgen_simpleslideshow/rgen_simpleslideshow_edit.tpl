...
<div id="<?php echo $module_key; ?>">
	...
	<div class="form-horizontal">
		...
		<div class="control-group">
			<label class="control-label">Custom HTML above module data</label>
			<div class="controls">
				<?php 
		...
		<tbody class="banner-container">
			...
			<tr class="table-header tbl-vm" id="row<?php echo $i; ?>">
				<td>
					...
					<label class="lbl-1">
						Caption
						<a class="helpbtn helpbtn1" data-type="page" data-title="Caption formating" data-info="view/image/rgen_theme/rgen_simpleslideshow.html">Formating info</a>
					</label>
					<?php foreach ($languages as $language) { ?>
					<?php 
						...
						$placeholder	= $language['name'] . ' - Enter title';
					?>
					<textarea placeholder="<?php echo $placeholder; ?>" name="<?php echo $name; ?>" cols="10" rows="4" style="background:url(view/image/flags/<?php echo $language['image']; ?>) no-repeat 420px 8px; padding-right: 40px; width: 400px; font-size:12px;"><?php echo $dbKey; ?></textarea>

					<!-- <input type="text" style="background:url(view/image/flags/<?php echo $language['image']; ?>) no-repeat 225px 8px; padding-right: 40px;" placeholder="<?php echo $placeholder; ?>" name="<?php echo $name; ?>" value="<?php echo $dbKey; ?>"><br> -->
					<?php } ?>

					<!-- <label class="lbl-1">Caption position</label>
					<?php 
						$ar 	= $cpPosition;
						$dbKey 	= isset($img_v['cpPosition']) ? $img_v['cpPosition'] : 'cc';
						$name	= $module_key.'[image_data]['.$i.'][cpPosition]';
						$id		= 'cpPosition'.$i;
					?>
					<span class='select' style="margin-bottom: 5px;">
						<select name="<?php echo $name; ?>" style="width: 254px;">
					...
					<label class="lbl-1">Caption settings</label>
					<div>
						<a data-url="view/template/module/<?php echo $modSettings['modKey']; ?>/cp_settings.php" data-title="Caption settings" class="popup btn mb10">Edit</a>
						<?php 
							$dbKey 	= isset($img_v['cpSettings']) ? $img_v['cpSettings'] : $cpSettings;
							$name	= $module_key.'[image_data]['.$i.'][cpSettings]';
						?>
						<input type="hidden" class="cpSettings" name="<?php echo $name; ?>" value="<?php echo $dbKey; ?>" />
					</div>

					<label class="lbl-1">URL</label>
					<?php 
						$ar 	= $urlType;
